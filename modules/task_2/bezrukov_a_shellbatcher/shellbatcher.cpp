...
#include "../../../modules/task_2/bezrukov_a_shellbatcher/shellbatcher.h"
...
void Batcher_ParallelMerge(int* vec, int partSize, int partsCount) {
    ...
    for (int i = 0; i <= partsCount - 1; i++) {
        ...
        Batcher_OddOrEvenMerge(partStartPtr, partSize, isOdd);
    }
...
void Shellsort_Parallel(std::vector<int>* vecPtr) {
    ...
    while (n / partSize != 1) {
        Batcher_ParallelMerge(vecData, partSize, partsCount);
